...
//reid
FaceReidentificator::FaceReidentificator(const std::string& pathToModel,
    bool doRawOutputMessages)
    : BaseDetection(pathToModel, doRawOutputMessages), enquedFaces(0) {
}

void FaceReidentificator::submitRequest() {
    if (!enquedFaces) return;
    request.set_input_tensor(ov::Tensor{ request.get_input_tensor(), {0, 0, 0, 0}, {enquedFaces, inShape[1], inShape[2], inShape[3]} });
    request.start_async();
    enquedFaces = 0;
...
void FaceReidentificator::enqueue(const cv::Mat& face) {
    if (!enabled()) {
        return;
    }
    ov::Tensor batch = request.get_input_tensor();
...
std::vector<float> FaceReidentificator::operator[](int idx) {
    std::vector<float> result;

    request.wait();
    const ov::Tensor& tensor = request.get_output_tensor();
    size_t n_featureSize = tensor.get_shape().at(1);
    const float* output = request.get_output_tensor().data<float>();
    auto outputIdxPos = output + idx * n_featureSize;

    for (auto i = 0; i < n_featureSize; i++)
    {
        result.push_back(outputIdxPos[i]);
    }

    return result;
}

std::shared_ptr<ov::Model> FaceReidentificator::read(const ov::Core& core) {
    slog::info << "Reading model: " << pathToModel << slog::endl;
    std::shared_ptr<ov::Model> model = core.read_model(pathToModel);
    logBasicModelInfo(model);
