...
void HeadPoseVisualizer::draw(cv::Mat& frame, cv::Point3f cpoint, HeadPoseDetection::Results headPose) {
    ...
    putHighlightedText(frame, out.str(), p2, cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(200, 10, 10), 1);
}
...
void Visualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar) {
    ...
    if (f->isAgeGenderEnabled()) {
        ...
        //out << "," << f->getAge();
        out << " Age: " << (int(f->getAge() + 5) / 10) * 10 << "s";
    }
    ...
    putHighlightedText(img, out.str(), textPos, cv::FONT_HERSHEY_SIMPLEX, 1, genderColor, 1);
